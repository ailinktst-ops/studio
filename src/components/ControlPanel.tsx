      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className="bg-yellow-500/10 border-yellow-500/20 backdrop-blur-md">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={resetRaffleHistory} title="Zerar quem já ganhou" className="h-8 w-8 p-0 text-yellow-500/40 hover:text-yellow-500"><RotateCcw className="w-3.5 h-3.5" /></Button>
          </CardHeader>
          <CardContent className="flex flex-col gap-3">
             <Button onClick={triggerRaffle} disabled={approvedParticipants.length < 2 || data.raffle?.isRaffling || loading} className="w-full bg-yellow-500 hover:bg-yellow-600 h-12 rounded-xl text-md font-black uppercase italic shadow-[0_0_15px_rgba(234,179,8,0.3)]">
              ...
              {data.raffle?.winnerId && !data.raffle?.isRaffling && (
                <Button onClick={clearRaffle} variant="outline" className="text-yellow-500 border-yellow-500/20 text-[10px] font-bold uppercase h-10"><Eraser className="w-4 h-4 mr-2" /> Limpar Overlay</Button>
              )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-purple-500/10 border-purple-500/20 backdrop-blur-md">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onClick={resetChallengeHistory} title="Zerar histórico de desafios" className="h-8 w-8 p-0 text-purple-500/40 hover:text-purple-500"><RotateCcw className="w-3.5 h-3.5" /></Button>
          </CardHeader>
          <CardContent className="flex flex-col gap-3">
             <Button onClick={triggerSurpriseChallenge} disabled={approvedParticipants.length < 1 || data.challenge?.isRaffling || loading} className="w-full bg-purple-600 hover:bg-purple-700 h-12 rounded-xl text-md font-black uppercase italic shadow-[0_0_15px_rgba(168,85,247,0.3)]">
              ...
              {data.challenge?.winnerId && !data.challenge?.isRaffling && (
                <Button onClick={clearChallenge} variant="outline" className="text-purple-500 border-purple-500/20 text-[10px] font-bold uppercase h-10"><Eraser className="w-4 h-4 mr-2" /> Limpar Overlay</Button>
              )}
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="main" className="w-full">
        ...
        <TabsContent value="main" className="space-y-6">
          ...
          <Card className="bg-card/30 border-blue-500/20">
            <CardHeader className="pb-2"><CardTitle className="text-sm font-black uppercase italic text-blue-500 flex items-center gap-2"><Music className="w-4 h-4" /> Playlist Ativa ({approvedMusic.length}/10)</CardTitle></CardHeader>
              ...
              {approvedMusic.length === 0 ? (
                ...
                approvedMusic.map((m) => (
                  <div key={m.id} className="bg-white/5 border border-white/5 p-3 rounded-xl flex items-center justify-between group">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => removeMusicRequest(m.id)} className="text-white/20 hover:text-destructive opacity-0 group-hover:opacity-100 h-8 w-8"><Trash2 className="w-3.5 h-3.5" /></Button>
                  </div>
                ))
              )}
            </CardContent>
            ...
            <CardContent>
              <form onSubmit={handleAddParticipant} className="flex gap-3">
                ...
                <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                  ...
                  <SelectContent className="bg-card border-white/10">{data.categories.map(cat => <SelectItem key={cat} value={cat}>{cat}</SelectItem>)}</SelectContent>
                </Select>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Select value={p.category} onValueChange={(val) => updateParticipantCategory(p.id, val)}>
                          ...
                          <SelectContent className="bg-card border-white/10">{data.categories.map(cat => <SelectItem key={cat} value={cat} className="text-[10px] font-bold uppercase">{cat}</SelectItem>)}</SelectContent>
                        </Select>
        ...
        <TabsContent value="moderation" className="space-y-6">
          <Card className="bg-card/30 backdrop-blur-md border-white/5">
            ...
            <CardContent className="space-y-4">{pendingParticipants.map(p => (
          ...
          <Card className="bg-card/30 backdrop-blur-md border-white/5">
            ...
            <CardContent className="space-y-4">{pendingMusic.map(req => (
              <div key={req.id} className="bg-white/5 border border-white/10 p-4 rounded-2xl flex items-center justify-between">
                <div><span className="block text-white font-black italic uppercase text-xs">{req.artist}</span><span className="block text-white/40 font-bold uppercase text-[10px] tracking-widest">{req.song}</span></div>
                <div className="flex gap-2">
          ...
          <Card className="bg-card/30 backdrop-blur-md border-white/5">
            ...
            <CardContent className="space-y-4">{pendingMessages.map(msg => (
              <div key={msg.id} className="bg-white/5 border border-white/10 p-5 rounded-2xl space-y-4">
                <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Badge className="bg-correio/20 text-correio border-none font-bold italic uppercase text-[10px]">De: {msg.from}</Badge><span className="text-white/20 text-xs">➔</span><Badge className="bg-secondary/20 text-secondary border-none font-bold italic uppercase text-[10px]">Para: {msg.to}</Badge></div></div>
                <p className="text-white font-medium italic text-lg">&ldquo;{msg.content}&rdquo;</p>
                <div className="flex gap-2"><Button onClick={() => moderateMessage(msg.id, 'approved')} className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold uppercase text-[10px] tracking-widest"><Check className="w-4 h-4 mr-2" /> Aprovar</Button><Button onClick={() => moderateMessage(msg.id, 'rejected')} variant="outline" className="flex-1 border-destructive/30 text-destructive hover:bg-destructive/10 font-bold uppercase text-[10px] tracking-widest"><Ban className="w-4 h-4 mr-2" /> Rejeitar</Button></div>
              </div>
            ))}</CardContent>
          </Card>
              ...
              <div key={joke.id} className="bg-white/5 border border-white/10 p-4 rounded-2xl flex items-center justify-between">
                 ...
                 <div className="flex gap-2"><audio id={`audio-${joke.id}`} src={joke.audioUrl} className="hidden" /><Button variant="outline" size="icon" onClick={() => (document.getElementById(`audio-${joke.id}`) as HTMLAudioElement).play()} className="h-9 w-9 bg-white/5 border-white/10 hover:bg-orange-500/20"><Volume2 className="w-4 h-4 text-orange-500" /></Button>
                    <Dialog open={editingJokeId === joke.id} onOpenChange={(open) => !open && setEditingJokeId(null)}>
                      <DialogTrigger asChild><Button variant="outline" size="icon" onClick={() => startEditingJoke(joke)} className="h-9 w-9 bg-white/5 border-white/10 hover:bg-white/10"><Edit className="w-4 h-4 text-white/40" /></Button></DialogTrigger>
                      <DialogContent className="bg-card border-white/10 backdrop-blur-xl"><DialogHeader><DialogTitle className="text-white font-black italic uppercase">Renomear Meme</DialogTitle></DialogHeader><div className="py-4"><Input value={editingJokeName} onChange={(e) => setEditingJokeName(e.target.value)} className="bg-black/40 border-white/10 h-12 font-bold" /></div><DialogFooter><Button onClick={handleSaveJokeName} className="w-full bg-orange-500 text-white font-black uppercase italic h-12">Salvar Nome</Button></DialogFooter></DialogContent>
                    </Dialog>
                    <Button onClick={() => triggerPiadinha(joke)} size="sm" className="bg-orange-500 hover:bg-orange-600 text-white font-bold uppercase text-[10px]"><Send className="w-4 h-4 mr-1" /> Enviar</Button><Button variant="ghost" size="icon" onClick={() => removeJoke(joke.id)} className="h-9 w-9 text-white/20 hover:text-destructive"><Trash2 className="w-4 h-4" /></Button></div>
              </div>
            ))}</CardContent>
          </Card>
        </TabsContent>
